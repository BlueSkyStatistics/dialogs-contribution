 ...
 * ---------------------------------------------------------------------------
 * 1. Every numeric input (n, power, standard deviation of covariate, hazard
...
class SampleSizeSurvivalCoxNew extends baseModal {
    static dialogId = 'SampleSizeSurvivalCoxNew'
    static t = baseModal.makeT(SampleSizeSurvivalCoxNew.dialogId)

    constructor() {
        var config = {
            id: SampleSizeSurvivalCoxNew.dialogId,
            label: SampleSizeSurvivalCoxNew.t('title'),
    ...
    res <- tryCatch({
        ...
        if (is.na(row$n)) {
            ...
            n_result <- ssizeEpiCont.default(
                power = row$power, theta = row$hr, sigma2 = row$sd ^ 2,
            ...
            power_result <- powerEpiCont.default(
                n = row$n, theta = row$hr, sigma2 = row$sd ^ 2,
                psi = row$propevent, rho2 = row$rsquared, alpha = alpha_touse
            )
        ...
        cat(sprintf(
            "Row %d failed (n=%s, power=%s, sd=%s, hr=%s, propevent=%s, rsquared=%s, siglevel=%s): %s\\n",
            i, row$n, row$power, row$sd, row$hr, row$propevent, row$rsquared, row$siglevel,
            conditionMessage(e)
        ))
        NULL
    })
...
if (is.null(results) || nrow(results) == 0) {
    ...
    if ({{selected.plotcurve | safe}}) {
        ...
        if (groupvar == "none" && nrow(plotDat) > 0) {
            candidateRaw <- c("n", "power", "sd", "hr", "propevent", "rsquared", "siglevel")
...
`
        ...
        var objects = {
			howtouse: {
				el: new labelVar(config, {
					label: SampleSizeSurvivalCoxNew.t('howtouse'), 
					style: "mb-3", 
					h:8
				})
			},
			n: {
				el: new input(config, {
					no: 'n',
					label: SampleSizeSurvivalCoxNew.t('n'),
			...
			power: {
				el: new input(config, {
					no: 'power',
					label: SampleSizeSurvivalCoxNew.t('power'),
			...
			sd: {
				el: new input(config, {
					no: 'sd',
					label: SampleSizeSurvivalCoxNew.t('sd'),
					style: "mt-5",
					required: true,
			...
			hr: {
				el: new input(config, {
					no: 'hr',
					label: SampleSizeSurvivalCoxNew.t('hr'),
					required: true,
			...
			propevent: {
				el: new input(config, {
					no: 'propevent',
					label: SampleSizeSurvivalCoxNew.t('propevent'),
					required: true,
			...
			rsquared: {
				el: new input(config, {
					no: 'rsquared',
					label: SampleSizeSurvivalCoxNew.t('rsquared'),
					required: true,
			...
			siglevel: {
				el: new input(config, {
					no: 'siglevel',
					label: SampleSizeSurvivalCoxNew.t('siglevel'),
					placeholder: "0.05",
					extraction: "TextAsIs",
					type: "text",
					allow_spaces: true,
			...
			alternativeopt: {
				el: new labelVar(config, {
					label: SampleSizeSurvivalCoxNew.t('alternativeopt'), 
					style: "mt-5", 
					h:5
				})
			},
			twosided: {
				el: new radioButton(config, {
					label: SampleSizeSurvivalCoxNew.t('twosided'),
					no: "altgrp",
					increment: "twosided",
					value: "2",
					state: "checked",
			...
			onesided: {
				el: new radioButton(config, {
					label: SampleSizeSurvivalCoxNew.t('onesided'),
					no: "altgrp",
					increment: "onesided",
					value: "1",
					state: "",
			...
			plotopt: {
				el: new labelVar(config, {
					label: SampleSizeSurvivalCoxNew.t('plotopt'),
					style: "mt-5",
					h:5
				})
			},
			plotcurve: {
				el: new checkbox(config, {
					no: 'plotcurve',
					label: SampleSizeSurvivalCoxNew.t('plotcurve'),
					state: "",
					extraction: "TextAsIs"
				})
			},
			curvex: {
				  el: new selectVar(config, {
					no: 'curvex',
					label: SampleSizeSurvivalCoxNew.t('curvex'),
					extraction: "NoPrefix|UseComma",
					options: ["n", "power", "sd", "hr", "propevent", "rsquared", "siglevel"],
					 default: "power",
					width:"w-50"
				})
			},
			curvegroup: {
				  el: new comboBox(config, {
					no: 'curvegroup',
					label: SampleSizeSurvivalCoxNew.t('curvegroup'),
					multiple: true,
					extraction: "NoPrefix|UseComma",
					options: ["none", "n", "power", "sd", "hr", "propevent", "rsquared", "siglevel"],
					default: "none",
					width:"w-50"
				})
			},
			mainTitle: {
                el: new input(config, {
                    no: "mainTitle",
                    label: SampleSizeSurvivalCoxNew.t('mainTitle'),
                    allow_spaces: true,
			...
			xlab: {
                el: new input(config, {
                    no: 'xlab',
                    label: SampleSizeSurvivalCoxNew.t('xlab'),
                    allow_spaces: true,
                    placeholder: "X Axis",
                    extraction: "NoPrefix|UseComma",
					value: ""
			...
			ylab: {
                el: new input(config, {
                    no: 'ylab',
                    label: SampleSizeSurvivalCoxNew.t('ylab'),
                    allow_spaces: true,
                    placeholder: "Y Axis",
                    extraction: "NoPrefix|UseComma",
					value: ""
			...
			legends: {
                el: new selectVar(config, {
                    no: 'legends',
                    label: SampleSizeSurvivalCoxNew.t('legends'),
                    multiple: false,
					extraction: "NoPrefix",
					required: false,
        ...
        const content = {
            ...
            nav: {
                name: SampleSizeSurvivalCoxNew.t('navigation'),
                icon: "icon-survival",
        ...
        this.help = {
            title: SampleSizeSurvivalCoxNew.t('help.title'),
            r_help: SampleSizeSurvivalCoxNew.t('help.r_help'), //Fix by Anil //r_help: "help(data,package='utils')",
            body: SampleSizeSurvivalCoxNew.t('help.body')
        }
;
    }
...
module.exports = {
    render: () => new SampleSizeSurvivalCoxNew().render()
}
